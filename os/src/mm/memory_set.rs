...
impl MemorySet {
    ...
    /// 首先在第 19 行，我们通过 new_bare 新创建一个空的地址空间，并在第 21 行通过 map_trampoline 为这个地址空间映射上跳板页面，
    /// 这是因为我们解析 ELF 创建地址空间的时候，并没有将跳板页作为一个单独的逻辑段插入到地址空间的逻辑段向量 areas 中，所以这里需要单独映射上。
    /// 剩下的逻辑段都包含在 areas 中。
    /// 我们遍历原地址空间中的所有逻辑段，将复制之后的逻辑段插入新的地址空间，
    /// 在插入的时候就已经实际分配了物理页帧了。
...
impl MapArea {
    ...
    // 是由于它还没有真正被映射到物理页帧上，所以 data_frames 字段为空。
